...
function draw() {
  ...
  image(backgroundImg, 0, cdm[5] / 5);

  for (let i = 0; i < 5; i++) {
    image(cloud, cdx[i] - 80, cdy[i] - 40 + cdm[i], 160, 80);
    if (cdy[i] + cdm[i] > 1040) {
      cdy[i] = 0;
      cdm[i] = 0;
    }
  ...
  if (dm) {
    if (t - st4 > 100) {
      image(playerexplosion, p.x - 30, p.y - 40, 60, 80);
      dmd++;
      st4 = t;
    }
    if (dmd === 5) {
...
class Player {
  ...
  display() {
    ...
    image(player, this.x - 30, this.y - 40, 60, 80);
...
class NormalEnemy {
  ...
  display() {
    image(enemy, this.x - 30, this.y - 40, 60, 80);
  }
}
